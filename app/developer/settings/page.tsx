        ...
        {params.created_user && (
          <div className="flex items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-emerald-400">
            ...
            <p className="text-sm font-semibold">
              User account registered successfully with active membership role!
            </p>
          </div>
        )}
        {params.error && (
            ...
            <Card className="border-white/10 bg-white/[0.02]">
              ...
              <CardContent>
                <form action={createMasterOrganizationAction} className="space-y-4">
                  ...
                  <Button
                    type="submit"
                    className="w-full bg-gradient-to-r from-accent-cyan to-accent-magenta text-black font-bold text-xs rounded-full py-2.5 hover:opacity-90 transition-all duration-300"
                  >
                    CREATE ORGANISATION
                  </Button>
                </form>
            ...
            <Card className="border-white/10 bg-white/[0.02]">
              ...
              <CardContent className="space-y-6">
                <form action={upsertGlobalSettingAction} className="grid gap-3">
                  ...
                  <Button
                    type="submit"
                    className="bg-white/10 hover:bg-white/20 text-white text-xs rounded-full"
                  >
                    Save Key
                  </Button>
                </form>
                ...
                <div className="grid grid-cols-2 gap-3 pt-4 border-t border-white/5">
                  <Link href="/developer/whatsapp">
                    <Button
                      variant="secondary"
                      className="w-full border-white/10 hover:bg-white/5 text-[10px] h-9 font-bold uppercase tracking-wider"
                    >
                      WhatsApp Config
                    </Button>
                  </Link>
              ...
              <CardContent>
                <form action={createMasterUserAction} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <option value="customer" className="bg-[#121218]">
                        Customer (Orders / Prints)
                      </option>
                      <option value="employee" className="bg-[#121218]">
                        Employee (Operator / Printer Shop staff)
                      </option>
                      <option value="admin" className="bg-[#121218]">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full h-11 bg-black/40 border border-white/10 text-white text-xs rounded-xl px-3 outline-none focus:border-accent-cyan"
                    >
                      <option value="" className="bg-[#121218]">
                        -- Auto-assigned default organization --
                      </option>
        ...
        <Card className="border-white/10 bg-white/[0.01]">
          <CardHeader>
            <CardTitle className="text-white text-sm">
              System Database Configuration Records
            </CardTitle>
          </CardHeader>
          <Table>
            <thead>
            ...
            <tbody>
              {settings.map((s) => (
                <tr key={s.id} className="border-b border-white/5">
                  ...
                  <Td className="text-xs text-white/50 truncate max-w-xs">
